angular.module('ETimetable').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('templates/courses.html',
    "<header class=\"header\"><a class=\"page-back\" ng-href=\"{{pageBack.href}}\"><i class=\"fa fa-chevron-left\"></i> {{pageBack.label}}</a><h1>コースを選択</h1></header><ul class=\"card-set\"><li ng-repeat=\"course in courses\"><a ng-href=\"{{course.href}}\" cls-hover=\"\" cls-active=\"\" class=\"card-item card-item-fixed card-item-shrinkable\"><abbr title=\"{{course.label}}\" class=\"card-item-icon\">{{course.abbr}}</abbr> <span class=\"card-item-content\">{{course.label}}</span> <span class=\"card-item-caret\"><i class=\"fa fa-chevron-right\"></i></span></a></li></ul>"
  );


  $templateCache.put('templates/departments.html',
    "<header class=\"header\"><a href=\"#/grades\" class=\"page-back\"><i class=\"fa fa-chevron-left\"></i> 学年を選択</a><h1>学科を選択</h1></header><ul class=\"card-set\"><li ng-repeat=\"department in departments\"><a ng-href=\"{{department.href}}\" cls-hover=\"\" cls-active=\"\" class=\"card-item card-item-fixed card-item-shrinkable\"><abbr title=\"{{department.label}}\" class=\"card-item-icon icon-{{department.abbr}}\">{{department.abbr}}</abbr> <span class=\"card-item-content\">{{department.label}}</span> <span class=\"card-item-caret\"><i class=\"fa fa-chevron-right\"></i></span></a></li></ul>"
  );
